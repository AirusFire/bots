...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Kullanım - **\`${GuildDB.prefix}play [şarkı]\``
      );
    ...
    if (player.state != "BAĞLANDI") await player.connect();
    ...
    try {
      if (SearchString.match(client.Lavasfy.spotifyPattern)) {
        ...
        if (Searched.loadType === "OYNATMA LİSTESİ YÜKLENDİ") {
          let songs = [];
        ...
        } else if (Searched.loadType.startsWith("PARÇA")) {
          player.queue.add(
        ...
        if (Searched.loadType === "EŞLEŞME YOK")
          return client.sendTime(
            message.channel,
            "** - için eşleşme bulunamadı **" + SearchString
          );
        else if (Searched.loadType == "OYNATMA LİSTESİ YÜKLENDİ") {
          ...
          SongAddedEmbed.addField("Müzisyen", Searched.tracks[0].author, true);
          SongAddedEmbed.addField(
            "Süre",
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (Searched.loadType) {
          ...
          case "ARAMA SONUÇLARI":
            ...
            SongAdded.addField("Müzisyen", Searched.tracks[0].info.author, true);
            if (player.queue.totalSize > 1)
              SongAdded.addField(
                "Sıradaki konumu",
                `${player.queue.size - 0}`,
        ...
        try {
          ...
          if (res.loadType === "YÜKLEME BAŞARISIZ") {
            if (!player.queue.current) player.destroy();
            return client.sendError(
              interaction,
        ...
        switch (res.loadType) {
          ...
          case "PARÇA YÜKLENDİ":
            ...
            SongAddedEmbed.addField("Müzisyen", res.tracks[0].author, true);
            SongAddedEmbed.addField(
              "Süre",
