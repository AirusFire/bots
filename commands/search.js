...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Kullanım - **\`${GuildDB.prefix}search [Sorgu]\``
      );
    ...
    if (player.state != "Bağlandı") await player.connect();

    let Searched = await player.search(SearchString, message.author);
    if (Searched.loadType == "EŞLEŞME YOK")
      return client.sendTime(
        message.channel,
        "Eşleşme bulunamadı " + SearchString
      );
    else {
      ...
      let msg = await message.channel.send(
        "**Çalmak istediğiniz şarkının numarasını yazın! '30 saniye' içinde sona erer.**"
      );
      ...
      SongAddedEmbed.addField("Müzisyen", `${Song.author}`, true);
        ...
        switch (res.loadType) {
          ...
          case "ARAMA SONUÇLARI":
            ...
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `Yazdığınız değer hatalı. Kullanım - \`(1-${max})\``
              );
